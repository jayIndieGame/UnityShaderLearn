...
public class MyLightingShaderGUI : ShaderGUI
{
	...
	void DoRenderingMode()
	{
		...
		if (EditorGUI.EndChangeCheck())
		{
			...
			{
                target.renderQueue = (int)settings.queue;
                target.SetOverrideTag("RenderType", settings.renderType);
                target.SetInt("_SrcBlend", (int)settings.srcBlend);
                target.SetInt("_DstBlend", (int)settings.dstBlend);
                target.SetInt("_ZWrite", settings.zWrite ? 1 : 0);
			}
		}
	}
	...
	void DoAlphaCutoff()
	{
		MaterialProperty slider = FindProperty("_AlphaClip");
